...
void rightSplit()//Grey Split - Right
	{
		...
		analogWrite(L,SPEED);
		delay(200);
		digitalWrite(LED2, LOW); //Turn off red Split LED
		while(digitalRead(S1) == LOW){//turn until the car is on track
			analogWrite(L, SPEED);
			digitalWrite(R, LOW);
		}
	}
...
1.9.7	Make Split Code more effective
*/
