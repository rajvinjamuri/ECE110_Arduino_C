...
void leftSharpTurn()
	{
		...
		analogWrite(L, 0);
		delay(200);//makes sure that the turn starts
...
////SPLIT/STOP
	int splitstop=X0+X1+X2+X3+X4+X5;//The logical sum of the sensors can be useful when testing for grey or stop conditions.
		//Stop
		if (splitstop>4 && splitstop<7)//more than three sensors see white and not a grey split
		{	
			endStop();
		}
		//Right Split
		...
		//Left Split
		if (X1*X2 && splitstop<7 && X0+X3==0)//two inner see white, no sensor sees grey, and two outer see black.
		{
			leftSplit();
		}
