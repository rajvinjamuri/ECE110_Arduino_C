...
//TAPE AVOIDING NAVIGATION
		if(X1 < 1.5)	{analogWrite(L, SPEED);}//Left Motor Control
			else			{analogWrite(L, 0);}
			
		if(X2 < 1.5)	{analogWrite(R, SPEED);}//Right Motor Control
			else			{analogWrite(R, 0);}
		
//OVERSHOOT TRACK NAVIGATION
		//Left Sharp
		if(X0 > 3.3 && X2 < 3.3){
			analogWrite(R, SPEED);
			digitalWrite(L, LOW);
		...
		if(X3 > 3.3 && X1 < 3.3){
			analogWrite(L, SPEED);
			digitalWrite(R, LOW);
			delay(20);//makes sure that the turn is complete
...
//SPLIT/STOP
		...
		if ((X4 > 1.5 && X4 < 3.3) || (X5 > 1.5 && X5 < 3.3)||(X1 > 1.5 && X1 < 3.3)||(X2 > 1.5 && X2 < 3.3)){//a sensor is at grey
			rightSplit();
		}
		else if (X1 > 1.5 && X2 > 1.5){//Both inside sensors detect white => Only split or stop scenarios
			//Stop
			if (X0 > 3.3 && X3 > 3.3 && X4 > 3.3 && X5 > 3.3){//all middle sensors see white
				endStop();
			}
...
1.8.8		Made Overshoot Track Navegation more efficient (emergency Tape avoiding)
*/
