...
void navigate()
	...
	//RIGHT ANGLE NAVIGATION
		...
		if(X0 > 3.3 && X1 > 3.3)
		{
			analogWrite(R, SPEED);
			analogWrite(L, 0);
			delay(500);//makes sure that the turn is complete
			while(X2 < 3.3){
				analogWrite(R, SPEED);
				analogWrite(L, 0);
			}
...
////SPLIT/STOP
		if (X1 > 1.5 && X2 > 1.5)//Both inside sensors detect white => Only split or stop scenarios
		{
			//Stop
